...
class NotificationManager: ObservableObject {
    ...
    // MARK: - Morning Check-In Notification

    /// Identifier for the single contextual check-in notification fired each morning.
    /// Deliberately distinct from "workout-<n>" reminders so upgrades don't collide.
    static let morningCheckInIdentifier = "morning-checkin"

    /// Replaces the placeholder morning reminder with contextual content
    /// pre-generated by `CheckInManager`. Scheduled for the user's reminder
    /// time on whichever day the check-in is for (today if still in future,
    /// else tomorrow).
    func updateMorningNotification(with checkIn: DailyCheckIn) {
        let center = UNUserNotificationCenter.current()

        let content = UNMutableNotificationContent()
        content.title = "\(checkIn.readinessLevel.emoji) \(checkIn.readinessLevel.label) · Morning Check-In"
        if let msg = checkIn.coachMessage, !msg.isEmpty {
            content.body = String(msg.prefix(240))
        } else if let workout = checkIn.workoutSummary {
            content.body = "Today: \(workout). Tap to chat with your coach."
        } else {
            content.body = "Tap to review your readiness and today's plan."
        }
        content.sound = .default
        content.userInfo = ["type": "morning_checkin"]
        content.categoryIdentifier = Self.morningCheckInIdentifier

        // Compute the next firing time at the user's reminder hour.
        let cal = Calendar.current
        let reminderComps = cal.dateComponents([.hour, .minute], from: reminderTime)
        guard let hour = reminderComps.hour, let minute = reminderComps.minute else { return }

        var triggerComps = cal.dateComponents([.year, .month, .day], from: Date())
        triggerComps.hour = hour
        triggerComps.minute = minute
        if let target = cal.date(from: triggerComps), target <= Date() {
            // Past — roll to tomorrow.
            triggerComps = cal.dateComponents([.year, .month, .day], from: cal.date(byAdding: .day, value: 1, to: Date()) ?? Date())
            triggerComps.hour = hour
            triggerComps.minute = minute
        }
        ...
        let request = UNNotificationRequest(
            identifier: Self.morningCheckInIdentifier,
            content: content,
            trigger: trigger
        )

        // Replace any previously scheduled check-in (identifier match overrides).
        center.removePendingNotificationRequests(withIdentifiers: [Self.morningCheckInIdentifier])
        center.add(request) { error in
            if let error = error {
                print("[NOTIFICATIONS] Morning check-in scheduling failed: \(error)")
            } else {
                print("[NOTIFICATIONS] Scheduled morning check-in")
            }
        }
    }
}
